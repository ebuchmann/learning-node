...
function typeAhead(search) {
  ...
  searchInput.on('input', async function () {
    ...
    try {
      var _ref = await _axios2.default.get('/api/search?q=' + this.value),
          results = _ref.data;

      if (results.length) {
        searchResults.innerHTML = (0, _dompurify.sanitize)(searchResultsHTML(results));
        return;
      }

      searchResults.innerHTML = (0, _dompurify.sanitize)('<div class="search__result">No results for ' + this.value + ' found!</div>');
    } catch (error) {
      console.error(err);
    }
  });
...
async function loadPlaces(map) {
  ...
  var _ref = await _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng),
      places = _ref.data;
